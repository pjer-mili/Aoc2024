...
def generate_matrix(robots: list[Robot], width, height, seconds_count):
    matrix = [["." for _ in range(width)] for _ in range(height)]
    for robot in robots:
        i, j = calculate_robot_position(robot, width, height, seconds_count)
...
def calculate_robot_position(robot: Robot, cols, rows, n):
    x = (robot.p[0] + robot.v[0] * n) % cols  # from left wall (j)
    y = (robot.p[1] + robot.v[1] * n) % rows  # from top wall (i)
    return y, x
...
def main():
    ...
    minimum_factor = sys.maxsize
    second = -1
    for i in range(101 * 103):
        matrix = generate_matrix(robots, 101, 103, i)
